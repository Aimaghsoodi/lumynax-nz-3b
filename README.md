...
[🤗 Weights & model card on Hugging Face](https://huggingface.co/AbteeXAILab/lumynax-nz-3b) · [abteex.com](https://abteex.com) · [lumynax.com](https://lumynax.com) · [Monorepo](https://github.com/Aimaghsoodi/lumynax-release)

---

This GitHub repo mirrors the **runtime scaffolding** (quickstart, requirements, Modelfile, Space, docs). The model **weights** are published on Hugging Face at `AbteeXAILab/lumynax-nz-3b` — they exceed GitHub's per-file limit.

```bash
hf download AbteeXAILab/lumynax-nz-3b --local-dir lumynax-nz-3b
cd lumynax-nz-3b && pip install -r requirements.txt && python quickstart.py --interactive
# Or:  pip install lumynax && lumynax run lumynax-nz-3b -i
```
